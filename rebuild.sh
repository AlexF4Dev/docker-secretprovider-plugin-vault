...
docker secret rm secret-zero multi_a multi_b multi_json secret wrapped_secret generic_vault_token
...
docker service create --constraint 'node.role == '$snitch_role --detach --name snitch --replicas 2 --env VAULT_ADDR=http://172.17.0.1:8200 --secret multi_a --secret multi_b --secret multi_json --secret secret --secret wrapped_secret --secret generic_vault_token vault sh -c 'echo -n "secret:              "; cat /run/secrets/secret; echo; echo -n "wrapped_secret:      "; cat /run/secrets/wrapped_secret; echo; echo -n "unwrapped_secret:    "; VAULT_TOKEN=$(cat /run/secrets/wrapped_secret) vault unwrap -field=value; echo; echo -n "generic_vault_token: "; cat /run/secrets/generic_vault_token; echo; echo -n "multi_a:             "; cat /run/secrets/multi_a; echo; echo -n "multi_b:             "; cat /run/secrets/multi_b; echo; echo -n "multi_json:          "; cat /run/secrets/multi_json; echo'
