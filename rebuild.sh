...
docker service create --name vault-helper-service --secret secret-zero --restart-condition on-failure busybox tail -f /dev/null
...
docker service create --detach --name snitch --secret haha --secret VAULT_TOKEN_SORT_OF busybox sh -c 'find /run/secrets -type f | xargs -i -n 1 sh -c "echo {}:; cat {}; echo"; sleep 2'
